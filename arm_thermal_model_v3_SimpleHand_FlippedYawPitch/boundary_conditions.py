...
BC_PRIORITIES = {
    ...
    'GEARBOX_HAND': 6,  # Added for special boundary condition
    'HOUSING_HAND': 6   # Added for special boundary condition
}
...
class ElementBoundary:
    ...
    def _is_in_boundary_region(self, x: float, y: float, z: float, bc_data: dict) -> bool:
        ...
        if bc_data["type"] == "ADIABATIC":
            source = bc_data["source_1"]
            # Get boundary condition region dimensions
            bc_x = source["centroid"]["x"]  # Already relative to center
            bc_y = source["centroid"]["y"]  # From bottom
            bc_z = source.get("centroid", {}).get("z", 0)  # From bottom
            bc_width = source["width"]
            bc_height = source["height"]
            bc_thickness = source.get("thickness", 0)  # Default to 0 if not specified
        ...
        if bc_data["type"] in ["CONST_Q", "ACTUATOR_CONNECTED"]:
