...
def create_compatible_meshes(geo_data):
    ...
    Uses region-specific dx, dy, dz if available, otherwise falls back to global settings.
    ...
    """
    # Get global mesh settings as fallback
    global_mesh_settings = geo_data.get("mesh_settings", {"dx": 10, "dy": 10, "dz": 0.5})
    regions = geo_data.get("regions", [])
    
    # Initialize mesh parameters dictionary
    mesh_params = {}
    
    print("\nMesh settings for each region:")
    
    # Calculate mesh parameters for each region
    for region in regions:
        region_id = region["id"]
        
        # Get region-specific mesh settings or fall back to global settings
        mesh_settings = region.get("mesh_settings", global_mesh_settings)
        dx = mesh_settings.get("dx", global_mesh_settings["dx"])
        dy = mesh_settings.get("dy", global_mesh_settings["dy"])
        dz = mesh_settings.get("dz", global_mesh_settings["dz"])
        
        print(f"\nRegion {region_id}:")
        print(f"  Element size: dx={dx:.2f} mm, dy={dy:.2f} mm, dz={dz:.2f} mm")
        
        # Calculate number of elements based on region dimensions
        Lx = region["width"]
        Ly = region["height"]
        Lz = region["thickness"]
        
        # Calculate number of elements needed (at least 2 points in each direction)
        nx = max(2, int(Lx / dx) + 1)
        ny = max(2, int(Ly / dy) + 1)
        nz = max(2, int(Lz / dz) + 1)
        
        # Calculate actual element sizes to fit region dimensions exactly
        region_dx = Lx / (nx - 1) if nx > 1 else Lx
        region_dy = Ly / (ny - 1) if ny > 1 else Ly
        region_dz = Lz / (nz - 1) if nz > 1 else Lz
        
        # Store region mesh parameters
        mesh_params[region_id] = {
            "nx": nx,
            "ny": ny,
            "nz": nz,
            "dx": region_dx,
        ...
        print(f"  Mesh dimensions: {nx}x{ny}x{nz} elements")
        print(f"  Actual element size: dx={region_dx:.2f} mm, dy={region_dy:.2f} mm, dz={region_dz:.2f} mm")
