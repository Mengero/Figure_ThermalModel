...
class ThermalParameters:
    ...
    def __init__(self, geo_data: Dict[str, Any]):
            ...
            if "boundary_conditions" in region:
                ...
                for bc in region["boundary_conditions"]:
                    ...
                    if bc_type == 'ADIABATIC':
                        p1 = bc.get('point1', {'x':0,'y':0})
                        p2 = bc.get('point2', {'x':0,'y':0})
                        print(f"    Adiabatic (line): ({p1['x']:.3f},{p1['y']:.3f}) -> ({p2['x']:.3f},{p2['y']:.3f})")
                        # For the line representation, print its extents (min/max) for info only
                        x_min = min(p1['x'], p2['x'])
                        x_max = max(p1['x'], p2['x'])
                        y_min = min(p1['y'], p2['y'])
                        y_max = max(p1['y'], p2['y'])
        ...
        for region in geo_data.get("regions", []):
            for bc in region.get("boundary_conditions", []):
                bc_type = bc.get("type", "UNKNOWN")
                if bc_type in ["CONST_Q", "CONVECTIVE", "CONVECTIVE_AIRGAP"]:
                    source_id = bc.get("id", f"{region['id']}_{bc_type}_{len(self.heat_sources)}")
