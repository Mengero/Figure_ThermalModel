...
def create_compatible_meshes(geo_data):
    ...
    Uses dx, dy, dz from mesh settings to determine element sizes.
    ...
    """
    mesh_settings = geo_data.get("mesh_settings", {"dx": 10, "dy": 10, "dz": 0.5})
    regions = geo_data.get("regions", [])
    
    # Initialize mesh parameters dictionary
    mesh_params = {}
    
    # Get element sizes from mesh settings
    dx = mesh_settings.get("dx", 10)
    dy = mesh_settings.get("dy", 10)
    dz = mesh_settings.get("dz", 0.5)
    
    print(f"Global element size: dx={dx:.2f} mm, dy={dy:.2f} mm, dz={dz:.2f} mm")
    ...
    for region in regions:
        ...
        print(f"Region {region_id} element size: dx={region_dx:.2f} mm, dy={region_dy:.2f} mm, dz={region_dz:.2f} mm")
