...
def update_matrix_with_geometries(A, metal_coords, params: ThermalParameters):
    """Update matrix A based on element positions (corner, edge, surface) for metal layer"""
...
def update_matrix_with_boundary_conditions(A, b, metal_coords, params: ThermalParameters, metal_boundary, mapping):
    ...
    for mapping_id, mapping_data in params.mappings_by_id.items():
        ...
        if mapping_id == "KNEE_ACTUATOR_CONNECTION":
            ...
            R_contact = params.mappings_by_id[mapping_id]["contact_resistance"]
            # Handle standard mappings
            for source_idx, target_idx in zip(mapping.region_mappings[source_key],
                                                             mapping.region_mappings[target_key]):
                # Set coupling terms
                A[source_idx, source_idx] -= 1 / (R_contact * dz_metal_m)
                A[source_idx, target_idx] = 1 / (R_contact * dz_metal_m)
                A[target_idx, target_idx] -= 1 / (R_contact * dz_metal_m)
                A[target_idx, source_idx] = 1 / (R_contact * dz_metal_m)
