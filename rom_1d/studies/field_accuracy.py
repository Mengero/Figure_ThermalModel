...
if a.csv:
    ...
    ax[0].grid(axis='x', alpha=.3); ax[0].legend(fontsize=8, ncol=4, loc='upper left')
    ax[1].set_ylabel('sim - exp [C]'); ax[1].set_ylim(-10, 10); ax[1].grid(axis='x', alpha=.3); ax[1].legend(fontsize=8, ncol=7)
    ax[2].set_ylabel('motor copper power [W]'); ax[2].set_xlabel('time [min]'); ax[2].grid(axis='x', alpha=.3); ax[2].legend(fontsize=8, ncol=7)
...
# ---------------- all cases ----------------
    ...
    ax.set_title('Field prediction accuracy per joint — UPS vs depal'); ax.grid(axis='x', alpha=.3); ax.legend(fontsize=8)
